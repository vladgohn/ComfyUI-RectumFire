...
> **更新**
> `Fire Resolve` 现在也支持 `rgthree Power Lora Loader` 和 `rgthree Lora Loader Stack`。
> 它会把这些节点里的 LoRA 条目与你本地可用的 LoRA 文件进行匹配，并给出逐行状态：
> 找到的是绿色，缺失的是红色。
> 这很重要，因为这两种 rgthree LoRA 加载节点在实际工作流里很常见，而部分 LoRA 缺失本来很容易被忽略。
...
> **重要说明**
> 在旧前端下它的行为是正常的，但 ComfyUI 新前端 / subgraph 的改动让 `Fire Banner` 目前变得不稳定，有时可用，有时行为不可预测。
> 这主要是上游前端变动带来的问题，并不是这个节点已经完成了新的适配。
> 等上游前端行为稳定下来之后，`Fire Banner` 会再做对应调整。
...
现在展开后的计时器 drawer 还会把最近 3 次运行结果直接保存在工作流里：
- 最终耗时
- 运行日期
- 本次运行使用的 GPU 名称和显存

这在回头查看工作流、比较不同结果，或者把工作流分享给别人时都很有用。
单看 `20 秒` 价值有限，但如果同时知道它是在什么 GPU 上跑出来的，这个时间就有上下文了。
...
> **更新**
> `Fire Done` 现在也内置了可选的显存清理。
> 新增了一个默认开启的 `clear_vram` 开关，所以在重型工作流里，这个节点现在可以在播放完成提示音的同时清理已占用的 VRAM。
> 这样做可以让工作流尾部更干净，尤其是视频生成或其他高显存占用任务里，`Done` 和显存清理本来就几乎总是一起出现。
